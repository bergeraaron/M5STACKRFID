...
/**/
  ...
  //for (int8_t blockOffset = 0; blockOffset < no_of_blocks; blockOffset--) {
    ...
    // Read block
    byteCount = sizeof(local_buffer);
    status = MIFARE_Read(blockAddr, local_buffer, &byteCount);
    if (status != STATUS_OK) {
      Serial.print(F("MIFARE_Read() failed: "));
      Serial.println(GetStatusCodeName(status));
      continue;
    ...
    if (isSectorTrailer) {
      c1  = local_buffer[7] >> 4;
      c2  = local_buffer[8] & 0xF;
      c3  = local_buffer[8] >> 4;
      c1_ = local_buffer[6] & 0xF;
      c2_ = local_buffer[6] >> 4;
      c3_ = local_buffer[7] & 0xF;
      invertedError = (c1 != (~c1_ & 0xF)) || (c2 != (~c2_ & 0xF)) || (c3 != (~c3_ & 0xF));
      g[0] = ((c1 & 1) << 2) | ((c2 & 1) << 1) | ((c3 & 1) << 0);
      g[1] = ((c1 & 2) << 1) | ((c2 & 2) << 0) | ((c3 & 2) >> 1);
      g[2] = ((c1 & 4) << 0) | ((c2 & 4) >> 1) | ((c3 & 4) >> 2);
  ...
  return;
} // End PICC_DumpMifareClassicSectorToBuffer()
